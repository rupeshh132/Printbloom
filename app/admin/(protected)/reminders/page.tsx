import { getUpcomingReminders, getAllReminders, markReminderSent } from "@/app/actions/reminders"
  ...
  return (
    <div className="max-w-6xl mx-auto">
              ...
              <div key={rem.id} className="bg-white p-4 border border-[#E0D9CF] rounded-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <form action={async () => {
                    ...
                    <button type="submit" className="text-xs text-[#8B6B43] hover:text-[#DFBC94] flex items-center gap-1">
      ...
      <div className="bg-white border border-[#E0D9CF] rounded-sm shadow-sm overflow-hidden">
        <h2 className="font-serif text-lg text-[#221F1C] p-6 border-b border-[#E0D9CF]">All Registered Occasions</h2>
        {all.length === 0 ? (
          <div className="p-8 text-center text-[#9A8F85]">
            No occasions tracked yet. Customers can add these from their account dashboard.
          </div>
        ) : (
          <table className="w-full text-left text-sm">
            <thead className="bg-[#F5F0E8] border-b border-[#E0D9CF]">
              <tr>
                <th className="px-5 py-3 font-mono text-[10px] text-[#9A8F85] uppercase tracking-wider font-normal">Customer</th>
                <th className="px-5 py-3 font-mono text-[10px] text-[#9A8F85] uppercase tracking-wider font-normal">Occasion</th>
                <th className="px-5 py-3 font-mono text-[10px] text-[#9A8F85] uppercase tracking-wider font-normal">Date (Every Year)</th>
                <th className="px-5 py-3 font-mono text-[10px] text-[#9A8F85] uppercase tracking-wider font-normal">Added On</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#E0D9CF]">
              {all.map((rem: any) => (
                <tr key={rem.id} className="hover:bg-[#FBF6EE]/50">
                  <td className="px-5 py-4">
                    <p className="font-medium text-[#221F1C]">{rem.customer_name}</p>
                    <p className="text-xs text-[#9A8F85]">{rem.phone_number}</p>
                  </td>
                  <td className="px-5 py-4 text-[#6D635B]">{rem.occasion_name}</td>
                  <td className="px-5 py-4 text-[#221F1C] font-medium">{formatDateWithoutYear(rem.occasion_date)}</td>
                  <td className="px-5 py-4 text-[#9A8F85] text-xs">{formatDate(rem.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
