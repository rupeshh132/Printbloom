...
const PATTERNS: Record<string, any> = {
  random: () => null,
  center: (x: number, y: number) => Math.hypot(x - 0.5, y - 0.5) / Math.SQRT1_2,
  edges: (x: number, y: number) => Math.min(x, 1 - x, y, 1 - y) * 2,
  'left-to-right': (x: number) => x,
  'right-to-left': (x: number) => 1 - x,
  'top-to-bottom': (_x: number, y: number) => y,
  'bottom-to-top': (_x: number, y: number) => 1 - y,
  diagonal: (x: number, y: number) => (x + y) / 2,
...
export function PixelSwap({
  ...
  useEffect(() => {
    ...
    frozenGrid.pixels.forEach((pixel: any, index: number) => {
      ...
      const timing = { duration: pixelMs, delay: pixel.offset * spread, easing: 'linear', fill: 'both' as FillMode };
  ...
  const renderLayer = (content: any, index: number) => {
